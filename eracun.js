...
var prijavljenaStranka = function(customerId, callback){
  pb.all("SELECT Customer.* FROM Customer WHERE CustomerId = " + customerId, 
  function(napaka, vrstice){
      callback(napaka, vrstice);
  })
}
    ...
    pesmiIzKosarice(zahteva, function(pesmi) {
      if (!pesmi) {
       ...
       // console.log(zahteva.session.izbranaStranka);
        prijavljenaStranka(zahteva.session.izbranaStranka, function(napaka, stranka){
          if(napaka){
            odgovor.sendStatus(500);
          }
          else{
